...
set -euo pipefail
cd "$(dirname "$0")/../.."
OUTPUT="${1:-build/local/aiFlow.app/Contents}"
...
for agent_arch in $AGENT_ARCHS; do
  ...
  swiftc -swift-version 5 -O -target "$agent_arch-apple-macosx14.0" -sdk "$SDK" \
    ...
    aiFlow/SecureShareCore.swift aiFlow/SecureShareRelay.swift aiFlow/QuickShareServer.swift aiFlow/QuickShareRuntime.swift aiFlowShareAgent/main.swift \
    -lsqlite3 -o "$agent_part"
  AGENT_PARTS+=("$agent_part")
done
lipo -create "${AGENT_PARTS[@]}" -output "$OUTPUT/MacOS/FinderFlowShareAgent"
cp aiFlowShareAgent/com.finderflow.share-agent.plist "$OUTPUT/Library/LaunchAgents/"
