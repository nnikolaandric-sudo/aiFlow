...
<sub>Built on <a href="https://github.com/Gtarafdar/FinderFlow">FinderFlow</a> by Gobinda Tarafdar (MIT). Release files, the bundle ID and folders on disk keep the FinderFlow name.</sub>
...
1. Download the latest **`FinderFlow-1.5.2.dmg`** from
...
- Every release ships a companion `FinderFlow-x.y.dmg.sha256` file (standard
  `shasum` format).
...
- The DMG’s `Fix Gatekeeper.command` only runs
  `xattr -dr com.apple.quarantine /Applications/FinderFlow.app` and opens the app.
